...
import 'package:woosh/models/cart_item.dart';
...
class _UpliftSaleCartPageState extends State<UpliftSaleCartPage>
  ...
  Future<void> _loadProducts() async {
    try {
      ...
      final products = sharedDataService.getProducts();
  ...
  Widget _buildCartItem(int index, UpliftCartItem item) {
    ...
    final totalPieces = (unitOfMeasure != null)
                ...
                Row(
                  ...
                  children: [
                    ...
                    IconButton(
                      ...
                      onPressed: () {
                        final newQuantity = item.quantity + 1;
                        final storeQuantity = item.product
                                .getQuantityForStore(widget.outlet.id) ??
                            0;
                        if (newQuantity <= storeQuantity) {
                          _cartController.updateItemQuantity(item, newQuantity);
                        } else {
                          Get.snackbar(
                            'Error',
      ...
      bottomNavigationBar: Obx(() {
        ...
        return Container(
          ...
          child: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              if (hasItems) _buildTotalSection(),
              const SizedBox(height: 16),
              SizedBox(
