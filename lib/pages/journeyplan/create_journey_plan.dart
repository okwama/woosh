...
import 'package:woosh/services/route/route_service.dart';
import 'dart:async';
...
class _CreateJourneyPlanPageState extends State<CreateJourneyPlanPage> {
  ...
  final ClockInOutState _clockState = Get.find<ClockInOutState>();
  ...
  void initState() {
    ...
    _initializeClients();
    _preloadRoutes(); // Preload routes for instant dropdown
    _scrollController.addListener(_onScroll);
    _checkClockStatus();
  }

  /// Check if user is clocked in before proceeding
  void _checkClockStatus() {
    if (!_clockState.isClockedIn.value) {
      Get.snackbar(
        'Clock In Required',
        'You must be clocked in to create a journey plan',
      ...
      // Optionally redirect to profile page to clock in
  ...
  Future<void> _initializeClients() async {
    if (_isInitialLoad) {
      ...
      try {
        // First, load from Hive cache for immediate display
        await _loadFromCache();

        // Use passed clients as initial data if cache is empty
        ...
        // If cache fails, use passed clients
        if (_allClients.isEmpty) {
          _allClients = widget.clients;
          filteredClients = _allClients;
        }
  ...
  Future<void> _preloadRoutes() async {
    try {
      // Preload routes in background for instant dropdown
      await RouteService.fetchRoutes();
    } catch (e) {
      // Silent fail - routes will load when dropdown is opened
    }
  }
  ...
  Future<void> _fetchFreshDataInBackground() async {
    try {
      ...
      // Save to Hive cache
      if (Get.isRegistered<ClientHiveService>()) {
        final clientHiveService = Get.find<ClientHiveService>();
        final clientModels = clients
      ...
      if (mounted) {
        setState(() {
          _allClients = clients.where((client) => client != null).toList();
          filteredClients = _allClients;
          _currentPage = 1;
        ...
        final actualCount = clients.length;
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Row(
              children: [
                ...
                Text(
                    'Updated with $actualCount client${actualCount != 1 ? 's' : ''}'),
              ],
            ),
      ...
      // Silent fail for background fetch - user already has data
    }
  }
  ...
  Future<void> _loadFromCache() async {
    try {
      if (!Get.isRegistered<ClientHiveService>()) {
        print('ClientHiveService not registered, skipping cache load');
        return;
      }
      ...
      if (cachedClients.isNotEmpty) {
        final cachedClientModels = cachedClients
            .map((clientModel) => Client(
                  id: clientModel.id,
                  name: clientModel.name,
                  address: clientModel.address,
                  contact: clientModel.phone,
                  email: clientModel.email,
                  latitude: clientModel.latitude,
                  longitude: clientModel.longitude,
                  regionId: 0,
                  region: '',
                  countryId: 0,
                ))
        ...
        setState(() {
          _allClients = cachedClientModels;
          filteredClients = cachedClientModels;
        });
      }
    } catch (e) {
    ...
    _debounce = Timer(const Duration(milliseconds: 500), () {
      if (mounted) {
        setState(() {
          ...
          if (query.isNotEmpty && _hasMoreData) {
  ...
  List<_ScoredClient> _matchAndScoreClients(
    ...
    for (final client in clients) {
      ...
      final contact = client.contact.trim().toLowerCase() ?? '';
      ...
      // Contains exact search query as a substring
        ...
        if (name.split(' ').any((word) => word == searchQuery) ||
            ...
            contact.split(' ').any((word) => word == searchQuery) ||
            email.split(' ').any((word) => word == searchQuery)) {
          score = 900.0;
        }
        // Boost score if it matches at start
        if (name.startsWith(searchQuery) ||
            address.startsWith(searchQuery) ||
            contact.startsWith(searchQuery) ||
      ...
      // Partial word match
        ...
        final contactWords = contact.split(' ');
        final emailWords = email.split(' ');

        int matchedWords = 0;
        for (final searchWord in searchWords) {
          if (nameWords.any((word) => word.contains(searchWord)) ||
              addressWords.any((word) => word.contains(searchWord)) ||
              contactWords.any((word) => word.contains(searchWord)) ||
              emailWords.any((word) => word.contains(searchWord))) {
            matchedWords++;
          }
        }
  ...
  Future<void> _loadMoreUntilFound(String query) async {
    ...
    bool foundMatch = _allClients.any((client) {
    ...
    while (!foundMatch && _hasMoreData) {
      await _loadMoreClients();

      foundMatch = _allClients.any((client) {
        final name = client.name.toLowerCase();
        final address = client.address?.toLowerCase() ?? '';
        final contact = client.contact.toLowerCase() ?? '';
        final email = client.email?.toLowerCase() ?? '';
    ...
    // Update filtered clients after loading more data
    _updateFilteredClients();
    setState(() {
      _isSearching = false;
    });
