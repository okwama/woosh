...
class PaginatedJourneyPlanResponse {
  ...
  factory PaginatedJourneyPlanResponse.fromJson(Map<String, dynamic> json) {
    ...
    // Debug logging for pagination fields (commented out to reduce log spam)
...
class JourneyPlanService {
  ...
  static Future<PaginatedJourneyPlanResponse> fetchJourneyPlans({
    ...
    try {
      ...
      final queryParams = <String, String>{
        ...
        'timezone': timezone,
        'date': today, // Always send today's date
      ...
      print('🔍 Fetching journey plans from: $uri');
      print('🔍 Query params: $queryParams');
      print('🔍 Today\'s date being sent: $today');
      ...
      if (response.statusCode == 200) {
        ...
        final result = PaginatedJourneyPlanResponse.fromJson(responseBody);
        print('🔍 Parsed journey plans: ${result.data.length} plans found');
        return result;
  ...
  static Future<void> deleteJourneyPlan(int journeyId) async {
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 204) {
        // Handle both 200 (with response body) and 204 (no content)
        if (response.body.isNotEmpty) {
          try {
            final decodedJson = jsonDecode(response.body);
            print(
                'Journey plan deleted successfully: ${decodedJson['message'] ?? 'Success'}');
          } catch (e) {
            print('Warning: Could not parse response body: $e');
          }
        } else {
          print('Journey plan deleted successfully (no content response)');
        }
        return;
      } else {
  ...
  static Future<PaginatedJourneyPlanResponse> getTodayJourneyPlans({
    ...
    try {
      ...
      print('🔍 Getting today\'s journey plans for date: $today');

      return await fetchJourneyPlans(
        page: page,
        limit: limit,
        timezone: timezone,
        date: today,
